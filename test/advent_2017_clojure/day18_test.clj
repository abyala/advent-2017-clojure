...
(def DUMMY_DUET (struct Duet {\a 10 \b 18 \c 0} 1 nil))
...
(deftest apply-command-tests
  (is (= (struct Duet {\a 10 \b 18 \c 0} 2 18)
         (apply-command DUMMY_DUET "snd b")))
  (is (= (struct Duet {\a 18 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "set a b")))
  (is (= (struct Duet {\a 5 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "set a 5")))
  (is (= (struct Duet {\a 15 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "add a 5")))
  (is (= (struct Duet {\a 28 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "add a b")))
  (is (= (struct Duet {\a 50 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "mul a 5")))
  (is (= (struct Duet {\a 180 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "mul a b")))
  (is (= (struct Duet {\a 1 \b 18 \c 0} 2 nil)
         (apply-command DUMMY_DUET "mod a 3")))
  (is (= (struct Duet {\a 10 \b 8 \c 0} 2 nil)
         (apply-command DUMMY_DUET "mod b a")))
  (is (= (struct Duet {\a 10 \b 18 \c 0} 2 5 nil)
         (apply-command (struct Duet {\a 10 \b 18 \c 0} 1 5)
                        "rcv c")))
  (is (= (struct Duet {\a 10 \b 18 \c 0} 2 5 5)
         (apply-command (struct Duet {\a 10 \b 18 \c 0} 1 5)
                        "rcv b")))
  (is (= (struct Duet {\a 10 \b 18 \c 0} 11 nil)
         (apply-command DUMMY_DUET "jgz b a")))
  (is (= (struct Duet {\a 10 \b 18 \c 0} 2 nil)
